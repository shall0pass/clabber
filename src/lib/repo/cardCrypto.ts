...
/** Apply `fn` to every secret card/seed string in a *copy* of `doc` and return
 *  the copy. `fn` sees each string once; it decides encrypt vs. decrypt. */
function mapSecretStrings(doc: AnyDoc, fn: (s: string) => string): AnyDoc {
	const d = clone(doc);

	if (Array.isArray(d.hands)) {
		d.hands = (d.hands as unknown[]).map((h) =>
			Array.isArray(h) ? h.map((c) => fn(c as string)) : h
		);
	}

	if (typeof d.seed === 'string' && d.seed !== '') d.seed = fn(d.seed);

	const melds = d.melds as AnyDoc | undefined;
	if (melds && Array.isArray(melds.declared)) {
		melds.declared = (melds.declared as unknown[]).map((seat) =>
			Array.isArray(seat)
				? seat.map((claim) => {
	...
	if (renege && Array.isArray(renege.couldHave)) {
...
/** A plaintext copy of `doc` with every secret card field decrypted. Values that
 *  aren't our blobs (a plain "AS", or a seed from an unencrypted game) pass
 *  through untouched, so old / half-migrated documents still render. A blob that
 *  fails to decrypt (wrong key) is left as-is rather than throwing. */
export function decryptDoc(doc: AnyDoc, key: Uint8Array): GameDoc {
	return mapSecretStrings(doc, (s) => {
